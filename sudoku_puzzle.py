...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        False
        """
        grid, symbol_set, n = self._grid, self._symbol_set, self._n
        for r in range(n):
            for c in range(n):
                used_symbols = ((self._row_set(r) | self._column_set(c)
                                 | self._subsquare_set(r, c)) - {' '})
                if grid[r][c] is EMPTY_CELL and symbol_set == used_symbols:
                    return True
        return False
    ...
    def has_unique_solution(self) -> bool:
        ...
        alt_solution = solver.solve(self, {str(solution)})
        return not alt_solution
